...
os.environ.setdefault("CHANNEL_CONFIG", str(Path(__file__).parent / "fixtures" / "test_channel_settings.json"))
os.environ.setdefault("CHANNEL_PLAYLIST_PATH", str(Path(__file__).parent / "fixtures" / "test_playlist.txt"))
os.environ.setdefault("CHANNEL_PLAYHEAD_PATH", str(Path(__file__).parent / "fixtures" / "test_playhead.json"))
os.environ.setdefault("CHANNEL_WATCH_PROGRESS_PATH", str(Path(__file__).parent / "fixtures" / "test_watch_progress.json"))
...
@pytest.mark.api
def test_update_channel_id_mismatch(client: TestClient, test_config_file: Path, monkeypatch):
...
@pytest.mark.api
def test_discover_shows(client: TestClient, test_config_file: Path, temp_dir: Path, monkeypatch):
    ...
    response = client.get(f"/api/channels/test-channel/shows/discover?media_root={media_root}")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) >= 2
...
@pytest.mark.api
def test_get_playlist_snapshot(client: TestClient, test_config_file: Path, test_playlist_file: Path, monkeypatch):
...
@pytest.mark.api
def test_update_playlist(client: TestClient, test_config_file: Path, test_playlist_file: Path, monkeypatch):
    ...
    if snapshot["upcoming"]:
        ...
        response = client.post("/api/channels/test-channel/playlist/next", json=update_request)
        assert response.status_code == 200
...
@pytest.mark.api
def test_update_playlist_version_mismatch(client: TestClient, test_config_file: Path, test_playlist_file: Path, monkeypatch):
...
def test_skip_current_episode(mock_subprocess: MagicMock, client: TestClient, test_config_file: Path, 
                               test_playlist_file: Path, test_playhead_file: Path, monkeypatch, temp_dir: Path):
